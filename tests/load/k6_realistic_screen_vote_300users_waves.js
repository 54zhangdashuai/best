...
const wave1Vus = Number(__ENV.VOTE_W1_VUS || 100);
const wave2Vus = Number(__ENV.VOTE_W2_VUS || 100);
const wave3Vus = Number(__ENV.VOTE_W3_VUS || 100);
...
export const options = {
  scenarios: {
    ...
    vote_wave_1: {
      ...
      vus: wave1Vus,
      iterations: 1,
      maxDuration: __ENV.VOTE_MAX_DURATION || '25s',
      startTime: __ENV.VOTE_W1_START || __ENV.VOTE_START_TIME || '15s',
      exec: 'voter'
    },
    vote_wave_2: {
      executor: 'per-vu-iterations',
      ...
      iterations: 1,
      maxDuration: __ENV.VOTE_MAX_DURATION || '25s',
      startTime: __ENV.VOTE_W2_START || '25s',
      exec: 'voter'
    },
    vote_wave_3: {
      executor: 'per-vu-iterations',
      ...
      iterations: 1,
      maxDuration: __ENV.VOTE_MAX_DURATION || '25s',
      startTime: __ENV.VOTE_W3_START || '35s',
      exec: 'voter'
    }
  },
  thresholds: {
...
export function voter(data) {
  ...
  const clientId = `wave_${String(__VU).padStart(4, '0')}_${randomString(10)}_${String(Date.now())}`;
